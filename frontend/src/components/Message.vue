                    ...
                    <el-table-column prop="messages">
                      <template slot-scope="scope">
                        <!--<div @click="viewDetail(scope.$index)" style="cursor: pointer;font-size: 24px;margin-bottom:20px;margin-top:10px" class="question">-->
                          ...
                          <!--{{scope.row.fromId}}-->
                        <!--</div>-->
                        <div v-show="false" style="font-family: Microsoft YaHei;font-size: 0.4rem;margin-left: 0.5rem;margin-bottom: 5px" class="content">
                          id：{{scope.row.id}}
                        </div>
                        <div style="font-family: Microsoft YaHei;font-size: 0.4rem;margin-left: 0.4rem;margin-bottom: 5px" class="content">
                          发信人：{{scope.row.fromId}}
                        </div>
                        <div style="font-family: Microsoft YaHei;font-size: 0.4rem;margin-left: 0.4rem;margin-bottom: 5px" class="content">
                          详细内容：{{scope.row.content}}
                        </div>
                        <div style="font-family: Microsoft YaHei;font-size: 0.4rem;margin-left: 0.4rem;margin-bottom: 5px" class="info1">
                          状态：
                          <span v-if="scope.row.hasRead == '0'">未读</span>
                          <span v-if="scope.row.hasRead == '1'">已读</span>
                        </div>
                        <div style="font-family: Microsoft YaHei;font-size: 0.3rem;margin-left: 0.3rem;margin-bottom: 5px" class="info1">
                          发信时间：{{scope.row.createTime}}
                        </div>
                        <div style="font-family: Microsoft YaHei;font-size: 0.3rem;margin-left: 0.3rem;margin-bottom: 5px" class="info3">
